...
const icsService: IIcsService = {
  getDuration: (dur: string): ics.DurationObject => {
    ...
    if(hourIndex !== -1) {
      if(hourIndex - 2 >= 0 && isNumber(+value.slice(hourIndex - 2, hourIndex))) {
    ...
    if(minuteIndex !== -1) {
      if(minuteIndex - 2 >= 0 && isNumber(+value.slice(minuteIndex - 2, minuteIndex))) {
    ...
    if(secondIndex !== -1) {
      if(secondIndex - 2 >= 0 && isNumber(+value.slice(secondIndex - 2, secondIndex))) {
        durationObject['seconds'] = +value.slice(secondIndex - 2, secondIndex)
      } else {
        durationObject['seconds'] = +value.slice(secondIndex - 1, secondIndex)
      }
